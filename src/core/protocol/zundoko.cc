...
#include <arpa/inet.h>
#include <pgen2/core/protocol/zundoko.h>
...
void zundoko_header::summary(bool moreinfo) const {
    printf("ZUNDOKO [type=");   
    print_type(type);
    printf("]\n");
...
void zundoko_header::write(void* buffer, size_t buffer_len) const {
...
void zundoko_header::read(const void* buffer, size_t buffer_len) {
    if (buffer_len < min_length) {
