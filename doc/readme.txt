...
プログラムのソースコードはこれで以上です。たった20数行のコードでパケットキャプチャプログラムの完成です。
ソースコードの説明は後にしてまずは実行してみましょう。

コンパイルはg++でコンパイルし、ライブラリのオプションとして -lpgenを追加してください。
libpgenはネットワークインターフェースを扱うときは常にraw socketを使用するので、送受信を行うプログラムの場合は
管理者権限で実行してください。
	$ g++ -o capture capture.cc -lpgen
	$ sudo ./capture wlan0

パケットキャプチャを体感できたところでソースコードを解説していきたいところですが、始めにlibpgenの基本的な
使い方について説明します。
...
### pingコマンドの作成
libpgenを使って簡単にパケットの受信、解析を行うことができました。
